...
const AddProduct = () => {
  ...
  useEffect(() => {
    fetch(`https://smartseosolution-f3192830bfae.herokuapp.com/products-variation`)
      .then((res) => res.json())
      .then((info) => setVariations(info));
  }, []);
  useEffect(() => {
    fetch(`https://smartseosolution-f3192830bfae.herokuapp.com/categories`)
      .then((res) => res.json())
  ...
  const handleAddProduct = (event) => {
    ...
    const url = `https://smartseosolution-f3192830bfae.herokuapp.com/add-product`;
    fetch(url, {
      method: "POST",
      headers: {
